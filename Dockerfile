# Multi-stage build for User Service with optimized caching
...
COPY pom.xml .
COPY iqscaffold-user-service/pom.xml iqscaffold-user-service/

# Download dependencies in separate layer for better caching
RUN mvn dependency:go-offline -pl iqscaffold-user-service -B

# Copy source code
COPY iqscaffold-user-service/src iqscaffold-user-service/src

# Build the application with optimizations
RUN mvn clean package -pl iqscaffold-user-service -DskipTests -B && \
...
# JVM optimization for user service workloads
